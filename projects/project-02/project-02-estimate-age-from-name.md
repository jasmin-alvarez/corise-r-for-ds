...
tbl_lifetables <- readr::read_csv(file, show_col_types = FALSE)
...
tbl_lifetables |> 
  ...
  #This is extra 
  # Added a smooth curve layer based on the sex (boy or       girl)of the person
  # Using a special method called "loess" to draw the curve   in a smooth way
  # The curve doesn't have any shaded area around it because   it was set se = FALSE
  ...
  labs(
    ...
    subtitle = "Based on the Year Born",
    x = "Year",
...
tbl_lifetables_extended_2022 |> 
  ...
  # Add labels (title, subtitle, x, y, caption)
    labs(
    title = "Survival Probability in 2022",
    subtitle = "Based on the Year Born",
    x = "Year",
    y = "Probability of Being Alive",
    caption = "Source: SSA"
...
plot_estimated_age <- function(tbl, my_name, my_sex) {
  tbl |> 
    ...
    geom_col(aes(y = nb_alive ), color = "darkgreen") +
...
``` r
 # Create `tbl_names_female_top_25` with name and sex  for top 25 females
  ...
  #Adding in a Step Arrange in descending order of total_births
...
tbl_names_extended_age |> 
  ...
  # Add labels (title, subtitle, x, y)
  labs(
    title = "Age Distribution of Top 25 Female Names",
    subtitle = "25th and 75th Percentiles",
    x = "Age",
    y = "Name"
  ) +
